...
export class FirebaseService {
  ...
  pushImagetoStorage(file: File) {
    ...
    if (file) {
      ...
      const uploadPercentage = this.storage.upload(filePath, file).percentageChanges();
      ...
      task.snapshotChanges().pipe(
        finalize(() => {
          ...
          this.downloadURL.subscribe((URL: string) => {
            const avtObj = new AvatarUpload(file.name, new Date().getDate().toString(), URL, fileExtension);
